...
function applyTransformScalar(value${s?": number":""}, axis${s?": 'x' | 'y' | 'avg'":""} = 'avg')${s?": number":""} {
	...
	return value * ((Math.abs(transformConfig.scaleX) + Math.abs(transformConfig.scaleY)) * 0.5);
}`}function Ue(e,t){const{vectorFormat:n,language:r,instanceMode:o=!1}=t,s=r==="typescript",i=n==="Processing",l=o&&(n==="createVector"||n==="Vec");return i?`void ${e}() {`:`function ${e}(${l?s?"p: any":"p":""})${s?": void":""} {`}function _e(e){const{vectorFormat:t,instanceMode:n=!1}=e;return n&&(t==="createVector"||t==="Vec")?"p.":""}function ne(e){const{vectorFormat:t,processingVector:n="PVector",language:r}=e;return t==="Processing"?n==="Vec2D"?"Vec2D":"PVector":t==="Vec"?"Vec":r==="typescript"?"p5.Vector":""}function U(e,t){if(e.length===0)return[];if(t.vectorFormat==="Processing")return[`${ne(t)} ${e.join(", ")};`];const n=t.language==="typescript"?`: ${ne(t)}`:"";return e.map(r=>`let ${r}${n};`)}function Q(e,t){return t.vectorFormat==="Processing"?`${ne(t)}[] ${e} = new ${ne(t)}[0];`:t.language==="typescript"?`let ${e}: ${ne(t)}[] = [];`:`let ${e} = [];`}function J(e,t,n){return n.vectorFormat==="Processing"?`${e} = new ${ne(n)}[] { ${t.join(", ")} };`:`${e} = [${t.join(", ")}];`}function Fe(e){return e.map(t=>`	${t}`).join(`
`)}function Le(){return{minX:Number.POSITIVE_INFINITY,minY:Number.POSITIVE_INFINITY,maxX:Number.NEGATIVE_INFINITY,maxY:Number.NEGATIVE_INFINITY}}function ie(e,t,n){e.minX=Math.min(e.minX,t),e.minY=Math.min(e.minY,n),e.maxX=Math.max(e.maxX,t),e.maxY=Math.max(e.maxY,n)}function Qe(e){return Number.isFinite(e.minX)&&Number.isFinite(e.minY)&&Number.isFinite(e.maxX)&&Number.isFinite(e.maxY)}function Je(e){const t=[];let n=[],r=!1,o=Le();const s=()=>{if(n.length===0)return;const i=Qe(o)?{...o}:{minX:0,minY:0,maxX:0,maxY:0};t.push({commands:n,closed:r,bounds:i}),n=[],r=!1,o=Le()};return e.forEach(i=>{if(i.type==="M"){s(),n.push(i),ie(o,i.x,i.y);return}if(i.type==="L"){n.length===0&&n.push({type:"M",x:i.x,y:i.y}),n.push(i),ie(o,i.x,i.y);return}if(i.type==="C"){if(n.length===0)return;n.push(i),ie(o,i.x1,i.y1),ie(o,i.x2,i.y2),ie(o,i.x,i.y);return}i.type==="Z"&&(r=!0,s())}),s(),t}function Me(e,t){return e.bounds.minX>=t.minX-1e-6&&e.bounds.maxX<=t.maxX+1e-6&&e.bounds.minY>=t.minY-1e-6&&e.bounds.maxY<=t.maxY+1e-6}function et(e,t,n,r,o,s,i,l){const c=t.vectorFormat==="Processing",a=_e(t),b=g=>H(g,n,r),T=[],E=[],I=[],p=[],C="'x'",f="'y'",d=c?"'a'":"'avg'",y=(g,m,x)=>{const $=`${i}_${g}`,v=`${s}${o}(${b(m)}, ${b(x)}))`;return E.push($),T.push(`${$} = ${v};`),$},w=(g,m,x)=>{const $=`applyTransformScalar(${b(m)}, ${x})`;c?I.push(`float ${g} = ${$};`):I.push(`const ${g} = ${$};`)};if(e.kind==="line"){if(e.x1==null||e.y1==null||e.x2==null||e.y2==null)return null;const g=y("p1",e.x1,e.y1),m=y("p2",e.x2,e.y2);return p.push(`${a}line(${g}.x, ${g}.y, ${m}.x, ${m}.y);`),{globalCode:[...U(E,t),Q(l,t)].join(`
`),assignments:[...T,J(l,E,t)],localDeclarations:I,drawCalls:p}}if(e.kind==="polyline"||e.kind==="polygon"){const g=e.points||[];return g.length<2?null:(g.forEach(([m,x],$)=>{y(`p${$}`,m,x)}),p.push(`${a}beginShape();`),E.forEach(m=>{p.push(`${a}vertex(${m}.x, ${m}.y);`)}),p.push(`${a}endShape(${e.kind==="polygon"?"CLOSE":"OPEN"});`),{globalCode:[...U(E,t),Q(l,t)].join(`
`),assignments:[...T,J(l,E,t)],localDeclarations:I,drawCalls:p})}if(e.kind==="rect"){if(e.x==null||e.y==null||e.width==null||e.height==null)return null;const g=y("rectPos",e.x,e.y);w("rectW",e.width,C),w("rectH",e.height,f);const m=e.rx||0,x=e.ry||0;if(m>0||x>0){if(Math.abs(m-x)>1e-9)return null;w("rectR",m,d),p.push(`${a}rect(${g}.x, ${g}.y, rectW, rectH, rectR);`)}else p.push(`${a}rect(${g}.x, ${g}.y, rectW, rectH);`);return{globalCode:[...U(E,t),Q(l,t)].join(`
`),assignments:[...T,J(l,E,t)],localDeclarations:I,drawCalls:p}}if(e.kind==="circle"){if(e.cx==null||e.cy==null||e.r==null)return null;const g=y("circleCenter",e.cx,e.cy);return w("circleDiameter",e.r*2,d),p.push(`${a}circle(${g}.x, ${g}.y, circleDiameter);`),{globalCode:[...U(E,t),Q(l,t)].join(`
`),assignments:[...T,J(l,E,t)],localDeclarations:I,drawCalls:p}}if(e.kind==="ellipse"){if(e.cx==null||e.cy==null||e.rx==null||e.ry==null)return null;const g=y("ellipseCenter",e.cx,e.cy);return w("ellipseW",e.rx*2,C),w("ellipseH",e.ry*2,f),p.push(`${a}ellipse(${g}.x, ${g}.y, ellipseW, ellipseH);`),{globalCode:[...U(E,t),Q(l,t)].join(`
`),assignments:[...T,J(l,E,t)],localDeclarations:I,drawCalls:p}}return null}function tt(e,t,n,r,o){const{vectorFormat:s,coordMultiplier:i,precision:l,processingVector:c="PVector",instanceMode:a=!1}=t,b=s==="Processing",T=b&&c==="Vec2D",E=a&&(s==="createVector"||s==="Vec"),I=o||`drawPath${n+1}`,p=b?T?"new Vec2D":"new PVector":s==="Vec"?"new Vec":a&&s==="createVector"?"p.createVector":"createVector",C=E&&s==="createVector"?"applyTransform(p, ":"applyTransform(",f=Ke(t),d=Ue(I,t),y=_e(t),w=`${I}Points`;if(r?.primitive){const F=et(r.primitive,t,i,l,p,C,I,w);if(F){const L=[...F.assignments,...F.localDeclarations,...F.drawCalls],A=`${d}
${Fe(L)}
}`;return{sharedCode:f,globalCode:F.globalCode,pathCode:A}}}const g=Pe(e),m=Je(g);let x=0;const $=[],v=[],X=F=>{const L=[];return F.commands.forEach(A=>{if(A.type==="M"||A.type==="L"){const j=`${I}_${te(x)}`,_=H(A.x,i,l),B=H(A.y,i,l);$.push(j),v.push(`${j} = ${C}${p}(${_}, ${B}));`),L.push(`${y}vertex(${j}.x, ${j}.y);`),x++;return}if(A.type==="C"){const j=`${I}_${te(x-1)}`,_=`${I}_${te(x)}`,B=j+"c",q="c"+_,ye=H(A.x1,i,l),$e=H(A.y1,i,l),ve=H(A.x2,i,l),Ce=H(A.y2,i,l),be=H(A.x,i,l),we=H(A.y,i,l);$.push(B,q,_),v.push(`${B} = ${C}${p}(${ye}, ${$e}));`),v.push(`${q} = ${C}${p}(${ve}, ${Ce}));`),v.push(`${_} = ${C}${p}(${be}, ${we}));`),L.push(`${y}bezierVertex(${B}.x, ${B}.y, ${q}.x, ${q}.y, ${_}.x, ${_}.y);`),x++}}),L};let k=[],V=!1,P=null,N=0;const se=[],re=()=>{k.length!==0&&(se.push([`${y}beginShape();`,...k,`${y}endShape(${V?"CLOSE":"OPEN"});`].join(`
`)),k=[],V=!1,P=null,N=0)};m.forEach(F=>{const L=X(F);if(L.length===0)return;if(k.length===0){k=L,V=F.closed,P=F.bounds,N=0;return}if(P!==null&&Me(F,P)){V=!0,k.push(`${y}beginContour();`),k.push(...L),k.push(`${y}endContour();`),N++;return}if(P!==null&&N===0&&Me({bounds:P},F.bounds)){const _=[...k];V=!0,P=F.bounds,k=[...L,`${y}beginContour();`,..._,`${y}endContour();`],N=1;return}re(),k=L,V=F.closed,P=F.bounds,N=0}),re();const ue=[...U($,t),Q(w,t)],G=se.map(F=>F.split(`
`).map(L=>`	${L}`).join(`
`)).join(`

`),de=[...v,J(w,$,t)],fe=de.length>0?`${Fe(de)}
          ...
          <div class="output path-section" id="shape-section-${h}">
            ...
            <div class="content-grid">
              ...
              <div class="code-container">
                <pre><code>${R(D)}</code></pre>
              </div>
            </div>
          </div>
        `});const G=v.trim(),de=se.join(`
...
`),F=[G,ce].filter(u=>u.length>0).join(`

`),L=l==="Processing"?"pde":c==="typescript"?"ts":"js",A=me("svg_complete",L),j=me("svg_paths",L),_=me("svg_shared",L),B=[];n&&y.forEach(u=>{const h=$.get(u.fileIndex)||[];if(h.length===0)return;const S=he(h,C,u.drawAllFunctionName).trim(),M=h.map(z=>re.get(z)||"").filter(z=>z.length>0).join(`

`).trim(),D=[S,M].filter(z=>z.length>0).join(`

`);B.push({codeKey:`drawing-file-${u.filePrefix}`,code:D,drawAllFunctionName:u.drawAllFunctionName,fileName:me(`svg_${yt(u.file.name,L)}`,L),sourceFileName:u.file.name})});const q={complete:F,drawing:ce,shared:G};B.forEach(u=>{q[u.codeKey]=u.code});const ye=`
      <div class="command-section">
        <div class="command-header">
          <h2>Complete File</h2>
          <div class="action-buttons">
            <button class="copy-btn" data-code-key="complete">Copy Complete Code</button>
            <button class="download-btn" data-code-key="complete" data-filename="${A}">Download ${A}</button>
          </div>
        </div>
    ...
    `,be=n?`
      <div class="combined-preview-section output">
        ...
        <div class="combined-preview-grid">
          ${y.map(u=>{const h=N.get(u.fileIndex);return!h||h.pathsData.length===0?"":`
    ...
    `;ee.innerHTML=we+ye+$e+ve+Ce+be+X,n?y.forEach(u=>{const h=N.get(u.fileIndex);!h||h.pathsData.length===0||De(h.pathsData,h.shapeIds,h.previewId,h.shapes)}):De(k,V,"preview-all",P),k.forEach((u,h)=>{ct(u,`preview-${h}`,P[h],I)}),ge=Ct(),ee.querySelectorAll(".download-btn[data-code-key]").forEach(u=>{u.addEventListener("click",()=>{const h=u.dataset.codeKey,S=u.dataset.filename;if(!h||!S)return;const M=q[h];if(!M)return;const D=new Blob([M],{type:"text/plain"}),z=URL.createObjectURL(D),K=document.createElement("a");K.href=z,K.download=S,K.click(),URL.revokeObjectURL(z);const Ge=u.textContent;u.textContent="Downloaded!",setTimeout(()=>{u.textContent=Ge},2e3)})}),ee.querySelectorAll(".copy-btn").forEach(u=>{u.addEventListener("click",async h=>{const S=h.currentTarget;if(S.dataset.copying==="1")return;const M=S.dataset.codeKey;let D="";if(M?D=q[M]||"":D=S.closest(".path-section")?.querySelector("code")?.textContent||"",!D){ze(S,"No code");return}S.dataset.copying="1";let z=!1;try{z=await $t(D)}finally{S.dataset.copying="0"}ze(S,z?"Copied!":"Copy failed")})})}function Ct(){const e=Array.from(ee.querySelectorAll(".shape-nav-link"));if(e.length===0)return()=>{};const t=e.map(l=>{const c=l.dataset.target;return c?document.getElementById(c):null}).filter(l=>l!==null);if(t.length===0)return()=>{};const n=l=>{e.forEach(c=>{c.classList.toggle("is-active",c.dataset.target===l.id)})},r=()=>{const l=window.innerHeight/2;let c=t[0],a=Number.POSITIVE_INFINITY;t.forEach(b=>{const T=b.getBoundingClientRect(),E=T.top+T.height/2,I=Math.abs(E-l);I<a&&(a=I,c=b)}),n(c)},o=l=>{const a=l.currentTarget.dataset.target;if(!a)return;const b=document.getElementById(a);b&&b.scrollIntoView({behavior:"smooth",block:"center"})};e.forEach(l=>{l.addEventListener("click",o)});let s=!1;const i=()=>{s||(s=!0,window.requestAnimationFrame(()=>{s=!1,r()}))};return window.addEventListener("scroll",i,{passive:!0}),window.addEventListener("resize",i),r(),()=>{e.forEach(l=>{l.removeEventListener("click",o)}),window.removeEventListener("scroll",i),window.removeEventListener("resize",i)}}
